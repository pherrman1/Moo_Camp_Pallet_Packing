...
This repository is configured for the intelligent Euro-pallet loading assignment. Start with [PROJECT_SETUP.md](PROJECT_SETUP.md), which documents installation, the local demo command, the Leap option, and the current modeling scope. The default command-line instance is [input/euro_pallet_demo.txt](input/euro_pallet_demo.txt).
